...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const companyId = 5; // NA RAZIE NA SZTYWNO

        const [invRes, taskRes, taxRes] = await Promise.all([
          axios.get(`http://localhost:3001/invoices/company/${companyId}`),
          axios.get(`http://localhost:3001/tasks`),
          axios.get(
            `http://localhost:3001/tax-obligations/company/${companyId}`,
          ),
        ]);
  ...
  const getStatusLabel = (doc: Document) => {
    switch (doc.status) {
      case "Opłacone":
        return doc.type === "Pismo urzędowe" ? "Zrealizowano" : "Zapłacono";
      case "Nieopłacone":
        return "Do opłacenia";
      case "Oczekuje":
  ...
  const aiSuggestions = [
    {
      ...
      details:
        "Twoje rozliczenie VAT za styczeń 2024 wynosi 2 340,50 PLN. Analiza przepływów wskazuje, że posiadasz wystarczające środki na koncie VAT. Rekomendujemy zlecenie przelewu już teraz, aby uniknąć kolejek w systemach bankowych 25-go dnia miesiąca.",
      actions: [
        "Zapłać teraz",
        "Ustaw przypomnienie na 24.02",
        "Pobierz JPK_V7M",
      ],
    ...
    },
    {
      ...
      details:
        "Klient XYZ spóźnia się z płatnością o 2 tygodnie. To trzecia taka sytuacja w ciągu ostatnich 6 miesięcy. AI wygenerowało już gotowy projekt wezwania do zapłaty z odsetkami ustawowymi.",
      actions: [
        "Wyślij wezwanie (Email)",
        "Zadzwoń do klienta",
        "Uruchom windykację polubowną",
      ],
    ...
    },
    {
      ...
      details:
        "Zgodnie z najnowszą nowelizacją przepisów dla sektora IT, Twoja firma kwalifikuje się do rozszerzonej ulgi na start. Możesz płacić obniżoną składkę przez kolejne 6 miesięcy. Wymaga to złożenia deklaracji ZUS ZZA z nowym kodem tytułu ubezpieczenia.",
      actions: [
        "Złóż wniosek przez PUE ZUS",
        "Skonsultuj z księgową",
        "Więcej o uldze",
      ],
  ...
  const upcomingDeadlines = taxes.slice(0, 3).map((tax: any) => {
    const dateObj = new Date(tax.dueDate);

    return {
      date: dateObj.getDate(),
      month: dateObj.toLocaleString("pl-PL", { month: "short" }),
      title: tax.type,
      detail: `Do zapłaty ${Number(tax.amount).toFixed(2)} PLN`,
  ...
  const chartData = React.useMemo(() => {
    ...
    const monthMap: Record<
      number,
      { month: string; income: number; costs: number }
    > = {};
    ...
    invoices.forEach((invoice: any) => {
      ...
      if (!monthMap[monthIndex]) {
        monthMap[monthIndex] = {
          month: date.toLocaleString("pl-PL", { month: "short" }),
          income: 0,
          costs: 0,
        };
      }

      monthMap[monthIndex].income += Number(invoice.amount || 0);
    });

    // sortujemy po numerze miesiąca
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {aiSuggestions.map((s, i) => (
            <div
              key={i}
              className={`${s.color} border p-6 rounded-[24px] flex flex-col justify-between dark:bg-slate-900/40 dark:border-slate-800 transition-all hover:shadow-md`}
            >
              <div>
                <div className="flex items-start gap-4 mb-4">
                  <div
                    className={`${s.iconColor} p-2 rounded-xl shrink-0 shadow-sm`}
                  >
                    {s.icon}
                  </div>
            ...
            <div className="flex justify-between items-center mb-8">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Podsumowanie przychodów i wydatków Twojej firmy
                </p>
              </div>
            ...
            <div className="h-[350px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  />
                  <Area
                    name="Koszty prowadzenia działalności"
                    type="monotone"
                    dataKey="costs"
                    stroke="#f43f5e"
                    strokeWidth={4}
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {invoices.slice(0, 4).map((invoice: any) => (
                    <tr
                      key={invoice.id}
                      onClick={() => onNavigateToDocument(String(invoice.id))}
                      className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer group"
                    >
                      <td className="px-8 py-5">
                        <p className="font-bold text-slate-800 dark:text-slate-200 group-hover:text-primary-600 transition-colors">
                      ...
                      <td className="px-8 py-5">
                        <span className="text-[10px] font-black px-2 py-1 rounded-lg border border-slate-200 text-slate-600 bg-slate-50">
                          Faktura
                        </span>
                      </td>
                      ...
                      <td className="px-8 py-5 font-bold text-slate-800 dark:text-slate-200">
                        {Number(invoice.amount).toFixed(2)} PLN
                      </td>

                      <td className="px-8 py-5 text-right">
                        <ChevronRight
                          size={18}
                          className="text-slate-300 group-hover:text-primary-500 transition-colors inline-block"
                        />
                      </td>
                    </tr>
                  ))}
          ...
          <div className="bg-white dark:bg-slate-900 p-8 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm">
            ...
            <div className="space-y-4">
              {upcomingDeadlines.map((d, i) => (
                ...
                >
                  <div
                    className={`w-12 h-12 ${d.color} rounded-xl flex flex-col items-center justify-center shrink-0 transition-transform group-hover:scale-105`}
                  >
                    <span className="text-[9px] font-black opacity-70 leading-none mb-0.5 uppercase">
                      {d.month}
                    </span>
            ...
            <div className="space-y-3">
              {tasks.slice(0, 4).map((task: any) => (
                ...
                >
                  ...
                  <span
                    className={`text-sm font-medium leading-tight ${task.status === "done" ? "text-slate-300 line-through italic" : "text-slate-700 dark:text-slate-300"}`}
                  >
                    {task.title}
                  </span>
                </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-950/40 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[40px] shadow-2xl border border-slate-100 dark:border-slate-800 p-8 flex flex-col gap-6">
            <div className="flex justify-between items-start">
              <div
                className={`${selectedAIHint.iconColor} p-3 rounded-2xl shadow-lg`}
              >
                {selectedAIHint.icon}
              </div>
              <button
                onClick={() => setSelectedAIHint(null)}
            ...
            <div className="space-y-3 pt-4 border-t border-slate-50 dark:border-slate-800">
              ...
              {selectedAIHint.actions.map((action: string, idx: number) => (
                <button
                  key={idx}
                  className="w-full flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 hover:bg-primary-600 hover:text-white rounded-2xl text-sm font-bold transition-all group"
                >
                  {action}
                  <ChevronRight
                    size={16}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-950/40 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[40px] shadow-2xl border border-slate-100 dark:border-slate-800 p-10 text-center flex flex-col items-center">
            ...
            <p className="text-slate-500 mb-8">
              Czy to zadanie zostało pomyślnie wykonane?
            </p>
...
function setSelectedTask(arg0: null) {
